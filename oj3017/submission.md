...
```text
โจทย์นี้ให้คำนวนค่าบิลที่ลูกค้าต้องจ่ายทั้งหมด โดยคิดว่า vat 7%และ บริการ 10% โดยคิดว่า vat จากราคารวมอาหารเครื่องดื่มแล้วค่าบริการแล้ว ซึ่งค่าบริการไม่ต่ำกว่า 50 และไม่เกิน 1,000

input:
รับค่าจำนวนเต็มเป็นราคาอาหารและเครื่องดื่ม

output:
แสดงผลรวมเงินทั้งหมดที่ลูกค้าต้องจ่ายโดยรวมค่า vat และ บริการแล้ว

constranits:
ควรตรวจสอบเรื่องเงื่อนไขของค่าบริการต้องกำหนดให้ตรวจสอบว่าถ้าต่ำกว่า 50 หรือสูงกว่า  1,000 ต้องปรับค่าให้เท่ากับค่าขั้นต่ำหรือสูงสุด
```
...
```text
Step 1: คิด service = price*0.1
Step 2: ถ้า service < 50 
Step 3: yes ให้ service = 50
Step 4: no ให้ไปตรวจสอบเงื่อนไขต่อไป
Step 5: เงื่อนไขต่อไป service > 1000
Step 6: yes ให้ service = 1000
Step 7: no ให้ service ราคา เท่าเดิม
Step 8: คิด vat = price*0.07
Step 9: คิดราคาที่ลูกค้าต้องจ่าย total = price + vat + service
Step 10 : แสดงผล total
```
...
```text
คล้ายกับวิธีแรกแต่มีการเปลี่ยนการคำนวน vat โดยให้ vat เป็นvat = (price+service)*0.07 แทนเพราะจริงๆ คิดจากราคาอาหารเครื่องดื่มรวมกับราคา service จากนั้นก็เหมือนเดิมอย่างแรกก็รับ ราคารวมเป็นจำนวนเต็ม สร้างตัวแปร service = price*0.1 จากนั้นก็ตรวจสอบเงื่อนไขว่า ถ้า service น้อยกว่ 50 ให้ service = 50 ถ้ามากกว่า 1000 ให้เท่ากับ 1000 แล้วก็ค่อยสร้างตัวแปร vat แล้วก็สร้างตัวแปรราคารวมโดยเอาราคาทุกอย่างมาบวกกัน total = price + vat + service จากนั้นก็แสดงผลโดยเป็นทศนิยมสองตำแหน่ง
```
...
```text
เพื่อทดสอบคำสั่ง if service < 50
```
...
```text
ทดสอบกรณีทั่วไปที่ Service Charge อยู่ระหว่าง 50 ถึง 1,000 บาท
```
...
```text
เพื่อทดสอบคำสั่ง elif service > 1000:
```
