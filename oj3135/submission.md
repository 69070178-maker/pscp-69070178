...
```text
โจทยให้กำหนดมามีคนจำนวน n คนแล้วมีขโมยคนที่ t และต้องส่งของขวัญข้ามคนไป k คนโดยต้องการให้หาจำนวนคนที่พิจารณาของขวัญซึ่งทุกคนที่ได้ส่งไปให้จะพิจารณาของขวัญก่อนส่งไปให้คนต่อไป แต่มีกรณีพิเศษคือลูปจะหยุดต่อเมื่อ 
1 .ขวัญวนกลับมาคนที่ 1 
2. ของขวัญวนมาเจอขโมยแล้วหยุดลูป( ถ้าเกิดกรณีนี้ต้องนับพิจารณาของขวัญเพิ่มไปอีก 1 ด้วย)

Input:
รับค่า n,k,t เป็นแถวเดียวกัน

Output:
จำนวนคนที่ได้พิจารณาของขวัญ

Constraints:
1 <= N <= 200,000; 1 <= K <= N-1; 1 <= T <= N) มีข้อมูลนำเข้า 50% ที่ N <= 300
```
...
```text
Step 1: รับค่า n , k , t
Step 2: กำหนดให้ current = 1
Step 3: กำหนดตัวแปร count = 1
Step 4:  เริ่มใช้ while loop วน
Step 5: ใน loop คำนวนหาตำแหน่งคนถัดไป
Step 6: ถ้าคนถัดไปเป้น คนที่ 1 หยุด loop
Step 7: เพิ่ม count  + 1 
Step 8: ให้ current = nextperson
Step 9:ถ้า current = t หยุด loop
Step 10: แสดง count
```
...
```text
คล้ายวิธีแรกแต่เพิ่มเงื่อนไขตอนปริ้นว่าถ้า t = 1 ให้แสดง 1 เลยแต่นอกจากนั้น print(count) เพราะโดน eage case 
```
