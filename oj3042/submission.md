...
```text
โจทย์นี้ให้เอาเลขมาหาเลขที่หารสิบลงตัว ตั้งแต่เลขที่ n ถึง 0

input:
รับจำนวนเต็มหนึ่งตัว

output:
เลขทั้งหมดที่หาร 10 ลงตัว

constraints:
output และ input ต้องเป็นจำนวนเต็มบวก
```
...
```text
Step 1: สร้างตัวแปร a รับค่าเลขที่ต้องการ
Step 2: สร้างตัวแปร my_list เป็น list
Step 3: วนเลข ในตัวแปร a โดยใช้ for loop ให้วนตั้งแต่ a ลดลงเรื่อยๆ จนถึง-1 แล้วหยุด 
Step 4: ถ้าเข้าเงื่อนไข if not i % 10 แล้วให้ ใช้ append i เข้า my_list ที่สร้างไว้
Step 5: ถ้านอกจากนั้น ให้ผ่านไป
Step 6: จากนั้นใช้แสดงผล my_list
```
...
```text
คล้ายกับแผนแรกแต่ว่าต้องเปลี่ยนวิธีการแสดงผล เราต้องการ unpacking operator เพราะเราไม่ต้องการ [] และ ,
ซึ่ง ก็ สร้างตัวแปร a รับค่าเลขที่ต้องการ สร้างตัวแปร my_list เป็น list วนเลข ในตัวแปร a โดยใช้ for loop ให้วนตั้งแต่ a ลดลงเรื่อยๆ จนถึง-1 แล้วหยุด  ถ้าเข้าเงื่อนไข if not i % 10 แล้วให้ ใช้ append i เข้า my_list ที่สร้างไว้
 ถ้านอกจากนั้น ให้ผ่านไป จากนั้นใช้แสดงผล my_list โดยปริ้นใช้ print(*my_list) เพื่อเอาเครื่องหมาย [] และ , ออก
```
...
```text
เลือกเลขที่ ไม่ได้หาร 10 ลงตัว เพื่อดูว่าสามารถหาเลขที่หาร 10 ลงตัวที่น้อยกว่าหรือเท่ากับ 57 ได้ถูก
```
...
```text
เลือกเลขที่ หาร 10 ลงตัวอยู่แล้ว เพื่อเช็กว่าแสดงเลข 100 ด้วย และแสดงเลขอื่น ลงมาจนถึง 0 ได้ครบ
```
...
```text
ลือกเลขที่ น้อยกว่า 10 เพื่อดูว่าเมื่อไม่มีเลขที่หาร 10 ลงตัว  โปรแกรมยังแสดงผลเป็น 0 ได้ถูก
```
