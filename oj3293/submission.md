...
```text
โจทย์ให้ทำเป็นกรอบสี่เหลี่ยมล้อมข้อความที่อินพุตมา

Input:
รับข้อความา 5 บรรทัด

Output:
ข้อความ 7 บรรทัดโดยจะมี space ระหว่างข้อความและกรอบซ้ายและขวาด้านละ  1 ช่อง 
กรอบซ้ายและกรอบขวาต้องห่างกันอย่างน้อย 2 space

Constraints:
ระวัง space หลังข้อความ
```
...
```text
Step 1:  loop รับ input มา 5 บรรทัด แล้วเอาไปใส่ใน list 
Step 2: แล้วทำ loop หาตัวที่ยาวที่สุด
Step 3: แล้วก็ใช้ len นับจำนวนตัวที่ยาวที่สสุด +4
Step 4: จากนั้นก็ ปริ้น * ตามจำนวน ตัวที่ยาว +5
Step 5: แล้วก็ loop ใส่ * กับ ข้อความ
```
...
```text
เหมือนกับแผนแรกแต่ต้องใช้ text.rstrip() ลบ space หลัง text ออก
```
...
```text
ทดสอบถ้ามีตัวที่ไม่ได้ใส่ตัวอักษรไป
```
...
```text
ทดสอบแบบไม่ใส่อะไรเลย
```

Input:

```text
จริง ๆ ต้องว่างแต่ใส่ไมได้
