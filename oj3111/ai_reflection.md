...
| OJ submission ID, if submitted | 626622 |
| OJ status | Pass |
...
```text
ให้ช่วยอธิบายโจทย์ว่าใช้ round_up ยังไง
```
...
```text
ช่วยให้สังเกตว่าเราต้องใช้แปลงรับ input มาเป็น Decimal เลยแล้ว Decimal ต้องคูณ กับ Decimal ด้วกันเองทำให้เลขออกมาถูกต้อง
```
...
```text
ได้แก้เรื่องรับ อินพุทและคำนวน เป็น decimalและแก้ตรงprint ให้เป็นทศนิยมสองหลักใช้ round_half_up
```
...
```text
ได้รู้วิธีใช้ round_half_up ได้รู้ฟังก์ชันใหม่ๆ ใน python 
```
