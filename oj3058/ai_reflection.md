...
| Item | Answer |
|---|---|
| OJ problem number/title | OJ3058 - [LEARNING LOGS] BrickBridge |
| OJ submission ID, if submitted | 600341	 |
| OJ status | Pass |
...
```text
ใหัช่วยดูโค้ดที่เขียนเสร็จแล้วไม่ผ่านเทสเคสเนื่องจาก timeout แล้วให้ช่วยใบ้ว่าใช้อะไรได้ ซึ่ง AI บอกว่าใช้ ฟังชั่น min() ได้จากนั้นก็นำมาประยุกต์ใช้เอง
```
...
```text
ช่วยให้สังเกตเห็นว่าเราใช้ฟังชั่นมาช่วยคิดได้
```
...
```text
แก้โค้ดจากใช้ loop มาเป็น ใช้สูตรแทนโดยใช้ฟังชั่น min
```
...
```text
เรียนรู้ว่าจริงๆ มีอีกหลายวิธีที่สามารถทำให้โค้ดไวขึ้นมีประสิทภาพขึ้นไม่จำเป็นต้องมีวิธีเดียวเราสามารถปรับแก้โค้ดให้มีประสิทธิภาพขี้นได้
```
