...
```text
โจทย์นี้ให้หาจำนวนเลขที่หารลงตัว แล้วเหลือเศษตามที่กำหนด

input:
จำนวนนับ A ที่น้อยกว่า B
จำนวนนับ B
จำนวนนับ d ซึ่งเป็นตัวหารที่ต้องการหาร
จำนวนนับ r ซึ่งเป็นเศษที่เราต้องการ

output:
จำนวนเลขที่หารด้วยตัวหาร d แล้วเหลือเศษ r

contraints:
ต้องระวังเรื่องประเภทของข้อมูลที่ input เข้าเนื่องจากกำหนดเป็นจำนวนนับซึ่งก็คควรใส่เป็น int
...
```text
แผนแรกของฉันคือใช้ for loop สำหรับไล่นับตั้งแต่ตัว a ถึง b ว่ามีเลขอะไรบ้างแล้วมีอะไรที่หารลงตัวแชล้วเหลือเศษที่ต้องการมั้ย ซึ่งใช้ % ในการเก็บเศษแล้วนำเศษไปเปรียบเทียบว่าตรงกับ r ที่เราต้องการมั้ยถ้าตรงให้บวกหนึ่งบไปที่ count แล้วก็ถ้าไม่ตรงก็ไม่ทำอะไร เมื่อจบ loop ก้ปริ้น จำนวนที่เรานับบวกเพิ่มไป
```
...
```text
เหมือนแผนแรกเลยก็คือรับค่าทุกอย่างมาเป็น int แล้วตั้งตัวแปร เก็บค่าเศษ กับ count นับจำนวนตัวที่เจอ จากนั้นใช้ ลูปโดยลุปเช็คเงือนไขใช้ if-else  แล้วก็แสดงผลออกมา
```
...
```text
เป็นการทดสอบการทำงานพื้นฐาน ในช่วง 10 ถึง 20
```
...
```text
ทดสอบกรณีเศษเป็น 0 (หารลงตัว) และค่าตรงกับขอบพอดี
```
